 ...
 * @param name タグの名前。
 */
export async function create(name: string): Promise<TagAPIResult> {
  // TODO: APIをぶっ叩く
 ...
 */
export async function find(name: string): Promise<TagAPIResult> {
  // TODO: APIをぶっ叩く
  await new Promise((resolver) => setTimeout(resolver, 50));
 ...
 */
export async function belongToProject(
  tagId: string,
  projectID: string
